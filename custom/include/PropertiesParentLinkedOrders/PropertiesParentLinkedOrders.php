...
class PropertiesParentLinkedOrders extends Link2
{/**
 * DB
 *
 * @var DBManager
 */
  ...
  public function buildJoinSugarQuery($sugar_query, $options = array())
  {
    ...
    $a  = $sugar_query->joinTable("listg_listings", ["joinType" => "INNER", "alias" => $jta]);

    $a->on()->equalsField($a->options["alias"] . ".id", $op->options["alias"] . ".parent_id");
    $a->on()->equals($a->options["alias"] . ".id", $this->focus->id);
    $a->on()->equals($op->options["alias"] . ".parent_type", "Listg_Listings");
